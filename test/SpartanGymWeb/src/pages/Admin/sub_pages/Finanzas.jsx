...
const Finanzas = () => {
  ...
  const cargarMovimientos = async () => {
    try {
      ...
      setError('No se pudo cargar finanzas desde la base de datos.');
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const nuevaTrx = await finanzasApi.crear({
        ...nuevoMovimiento,
      ...
      setError('No se pudo guardar el movimiento en la base de datos.');
    } finally {
  ...
  const ingresos = transacciones.filter((t) => t.tipo === 'ingreso').reduce((acc, t) => acc + Number(t.monto), 0);
  const egresos = transacciones.filter((t) => t.tipo === 'gasto').reduce((acc, t) => acc + Number(t.monto), 0);
  ...
  return (
    <div className="flex flex-col gap-6 text-white min-h-screen pb-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <MetricCard title="Balance" value={`$${balance.toFixed(2)}`} icon={DollarSign} color="text-gray-400" trend="+12%" />
        <MetricCard title="Ingresos" value={`$${ingresos.toFixed(2)}`} icon={TrendingUp} color="text-green-500" trend="84 Trx" />
        <MetricCard title="Egresos" value={`$${egresos.toFixed(2)}`} icon={TrendingDown} color="text-red-500" trend="Gastos" />
        <MetricCard title="Pendiente" value={`$${transacciones.length}`} icon={CreditCard} color="text-orange-500" trend="17 Socios" />
      </div>
                ...
                {transaccionesFiltradas.map((t) => (
                  <tr key={t.id} className="hover:bg-white/[0.02]">
                    <td className="p-4">
                      ...
                      <span className="text-[10px] text-gray-500">{new Date(t.fechaTransaccion).toLocaleString()}</span>
                    </td>
                    <td className="p-4">
