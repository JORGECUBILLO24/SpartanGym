...
const Reportes = () => {
  ...
  useEffect(() => {
    reportesApi.resumen()
      .then(setMetricas)
      .catch(() => setApiError('No se pudo cargar el resumen desde la API. Se muestran métricas de referencia.'));
  }, []);
  ...
  const generarReporte = async (id) => {
    ...
    try {
      const nuevoReporte = await reportesApi.generar(id);
      ...
      setApiError('No se pudo generar el reporte. Verifica que la API esté activa y que tu sesión sea válida.');
    } finally {
      setGenerandoId(null);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 text-white min-h-screen pb-10">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <AnalyticCard title="Socios" value={metricas ? metricas.totalSocios : "+24%"} desc="Registrados" icon={TrendingUp} color="text-green-500" />
        <AnalyticCard title="Retención" value={metricas ? `${metricas.tasaRetencion}%` : "92.3%"} desc="Socios activos" icon={Users} color="text-red-500" />
        <AnalyticCard title="Asistencias" value={metricas ? metricas.asistenciasRegistradas : "4.2x"} desc="Accesos registrados" icon={Package} color="text-orange-500" />
        <AnalyticCard title="Margen Neto" value={metricas ? `${metricas.margenNeto}%` : "82.4%"} desc="Eficiencia caja" icon={BarChart3} color="text-blue-500" />
      </div>

      {apiError && (
        <div className="rounded-xl border border-yellow-500/30 bg-yellow-500/10 px-4 py-3 text-xs font-semibold text-yellow-100">
          {apiError}
        </div>
      )}
